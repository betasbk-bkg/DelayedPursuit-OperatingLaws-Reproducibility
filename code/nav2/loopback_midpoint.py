...
class LoopbackSimulator(Node):
    ...
    def timerCallback(self):
        ...
        # ---- this study DIAGNOSTIC PATCH, one line of physics --------------------
        # The upstream node advances the position using the heading from BEFORE
        # this tick and only then rotates:
        #     x += dx*cos(yaw);  ...;  rotation = addYawToQuat(rotation, dth)
        # Over a tick with constant (v, omega) the body actually travels along an
        # arc whose mean heading is yaw + omega*dt/2, so using yaw alone lags the
        # displacement direction by half a tick of heading -- an extra T_sim/2 on
        # top of the zero-order hold, present only where omega != 0.
        #
        # E10 measured the T_sim/2 coefficient at 1.2626 [1.177, 1.544] where
        # Theorem 2 requires 1.  This build uses the MIDPOINT heading instead.
        # If the coefficient falls to ~1, the excess is this integration scheme
        # and not the theory.  It is a DIAGNOSTIC: the headline results stay on
        # the unmodified upstream simulator.
        yaw_mid = yaw + 0.5 * dth
