...
class LoopbackSimulator(Node):
    ...
    def __init__(self):
        ...
        # ---- this study DIAGNOSTIC PATCH: first-order actuator lag ---------------
        # The shipped plant is an ideal integrator: cmd_vel is applied at once.
        # A reviewer's standing objection to this validation is exactly that --
        # "the plant has no dynamics".  Rather than answer it with an
        # uncontrolled physics engine, we ADD THE OMITTED TERM and sweep it.
        #
        # A first-order lag  vdot = (u - v)/T  has impulse response
        # (1/T)exp(-t/T), whose FIRST MOMENT IS EXACTLY T.  Theorem 2 composes
        # mean delays, so it predicts, with coefficient 1 and no free parameter,
        #     tau_tot = tau + T_ctrl/2 + T_sim/2 + T_act.
        # Measuring that coefficient tests Theorem 2 on a term it has never been
        # tested on, and on a real actuator rather than a discretisation.
        #
        # Discretisation: alpha = 1 - exp(-dt/T) is the EXACT zero-order-hold
        # response of the continuous lag, so the added mean delay is T for any
        # tick size -- no dt-dependent bias creeps into the measured coefficient.
        # actuator_tau = 0 disables the filter and reproduces upstream exactly.
        self.declare_parameter('actuator_tau', 0.0)
        self.actuator_tau = \
            self.get_parameter('actuator_tau').get_parameter_value().double_value
        self.act_alpha = (1.0 - math.exp(-self.update_dur / self.actuator_tau)
                          if self.actuator_tau > 0.0 else 1.0)
        self.applied_cmd_vel = Twist()
        self.get_logger().info('this study actuator lag T_act = %.4f s (alpha = %.4f)'
                               % (self.actuator_tau, self.act_alpha))
    ...
    def timerCallback(self):
        ...
        # First-order actuator lag on every commanded component.  With
        # actuator_tau = 0 this is a plain assignment and the node is upstream.
        a = self.act_alpha
        self.applied_cmd_vel.linear.x += \
            a * (self.curr_cmd_vel.linear.x - self.applied_cmd_vel.linear.x)
        self.applied_cmd_vel.linear.y += \
            a * (self.curr_cmd_vel.linear.y - self.applied_cmd_vel.linear.y)
        self.applied_cmd_vel.angular.z += \
            a * (self.curr_cmd_vel.angular.z - self.applied_cmd_vel.angular.z)

        # Update odom->base_link from the APPLIED velocity
        dx = self.applied_cmd_vel.linear.x * self.update_dur
        dy = self.applied_cmd_vel.linear.y * self.update_dur
        dth = self.applied_cmd_vel.angular.z * self.update_dur
        ...
        self.t_odom_to_base_link.transform.translation.x += \
            dx * math.cos(yaw) - dy * math.sin(yaw)
        self.t_odom_to_base_link.transform.translation.y += \
    ...
    def publishOdometry(self, odom_to_base_link):
        ...
        odom.twist.twist = self.applied_cmd_vel
