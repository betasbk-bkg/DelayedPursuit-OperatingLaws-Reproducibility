...
class LoopbackSimulator(Node):
    ...
    def __init__(self):
        ...
        # ---- this study DIAGNOSTIC PATCH: Ackermann steering bound ---------------
        # kappa_max = tan(max_steer) / wheelbase.  max_steer <= 0 disables the
        # bound and the node is the shipped plant, bit for bit.
        self.declare_parameter('wheelbase', 0.5)
        self.declare_parameter('max_steer', 0.0)
        self.wheelbase = \
        ...
        self.kappa_max = (math.tan(self.max_steer) / self.wheelbase
                          if self.max_steer > 0.0 else 0.0)
        self.applied_cmd_vel = Twist()
        self.get_logger().info(
            'this study bicycle: wheelbase %.3f m, max_steer %.4f rad -> '
            'kappa_max %.4f 1/m (min radius %s m)'
            % (self.wheelbase, self.max_steer, self.kappa_max,
               ('%.3f' % (1.0 / self.kappa_max)) if self.kappa_max > 0
               else 'unbounded'))
    ...
    def timerCallback(self):
        ...
        # ---- this study DIAGNOSTIC PATCH: Ackermann curvature saturation --------
        # The commanded angular rate is realised through a steering angle,
        #     delta = atan(omega W / v),   omega = v tan(delta) / W
        # which is an identity until the steering angle hits its stop.  Clipping
        # delta at max_steer therefore bounds the CURVATURE at
        #     kappa_max = tan(max_steer) / wheelbase
        # and that bound is the only thing separating this plant from the
        # shipped unicycle.  With max_steer wide the two are identical, which is
        # what the self-test checks.
        self.applied_cmd_vel.linear.x = self.curr_cmd_vel.linear.x
        self.applied_cmd_vel.linear.y = self.curr_cmd_vel.linear.y
        v = self.curr_cmd_vel.linear.x
        w = self.curr_cmd_vel.angular.z
        if self.kappa_max > 0.0 and abs(v) > 1e-6:
            kappa = w / v
            if abs(kappa) > self.kappa_max:
                kappa = math.copysign(self.kappa_max, kappa)
            self.applied_cmd_vel.angular.z = kappa * v
        else:
            self.applied_cmd_vel.angular.z = w
