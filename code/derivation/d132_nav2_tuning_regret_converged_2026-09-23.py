#!/usr/bin/env python3
"""
d132 -- d128 re-run with the converged plant-class constants of d131.

Identical to d128 in every other respect; only U_CURV and U_HEAD change (0.306034 -> 0.307831 and
0.499496 -> 0.498429), because those were Euler values quoted to six digits.  d128 stays in the
record as what was run before.

d128 -- does the operating law give a practical tuning benefit on the Nav2 data?

The Nav2 validation so far shows that the curvature-command constant u* = 0.307831 is
...
For every stored Nav2 speed-RMSE curve (no new runs):
  law:        v_law = u* L / tau_tot, u* = 0.307831, tau_tot the first-order budget with
...
U_CURV, U_HEAD, U_C = 0.307831, 0.498429, 0.520494   # d131: converged, superseding d38's Euler values
...
def main():
    ...
    res["curves"] = rows_all
    json.dump(res, open(os.path.join(HERE, "d132_nav2_tuning_regret_converged_2026-09-23.json"), "w"), indent=1, default=float)
    print("\n-> d132_nav2_tuning_regret_converged_2026-09-23.json")
